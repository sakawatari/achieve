class SubmitRequestsController < ApplicationController
  ...
  def approve
    if @submit_request.update(status: params[:submit_request][:status].to_i)
  ...
  def create
    ...
    respond_to do |format|
      if @submit_request.save
        format.html { redirect_to submit_requests_url, notice: '依頼を送信しました。' }
      else
  ...
  def update
    respond_to do |format|
      if @submit_request.update(submit_request_params)
        format.html { redirect_to submit_requests_url, notice: '依頼を編集しました。' }
      else
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to submit_requests_url, notice: '依頼を削除しました。' }
    end
  end
